...
const OutputPanel: React.FC<OutputPanelProps> = ({
  ...
  if (loading) {
    return (
      <section
        className="border p-4 min-h-[300px] flex items-center justify-center"
        style={{
    ...
    return (
      ...
      >
        ...
        <div className="text-center py-4">
          <div
            className="mb-4 p-3 border border-red-600 inline-block text-red-700"
    ...
    >
      <h2 className="text-xl mb-4 relative inline-block">
        <span className="relative z-10">
          Predicted Mix Designs{displayResult?.mocked ? ' (Mocked)' : ''}
        </span>
        {isNotebook && (
          <span
        ...
        {isConstruction && (
          <span
            ...
            style={{
              backgroundImage:
                'repeating-linear-gradient(45deg, #fbbf24, #fbbf24 5px, #000000 5px, #000000 10px)',
              zIndex: 0,
            }}
          />
        )}
      ...
      <div className="space-y-4">
        {displayResult.predictions.map((prediction, index) => (
          ...
          >
            <div
              className="flex justify-between items-start mb-3 pb-2 border-b gap-4"
              style={{ borderColor: 'var(--theme-panel-border)' }}
            >
              <div className="flex-1 min-w-0">
                {(() => {
                  const rockName =
                    (rockDb as RockDb)[prediction.rock_id]?.name || `Rock ${prediction.rock_id}`
                  const mortarName =
                    (mortarDb as MortarDb)[prediction.mortar_id]?.name || prediction.mortar_id
                  return (
                    <>
              ...
              {(() => {
                ...
                if (!mortarMeta) {
                  return (
                    ...
                    >
                      Mortar information not available
                    </div>
                  )
                }
                    ...
                    <table className="w-full text-sm">
                      <tbody>
                        <tr>
                          ...
                          <td className="py-1" style={{ color: 'var(--theme-panel-text)' }}>
                            {mortarName}
                          </td>
                        </tr>
                        <tr
