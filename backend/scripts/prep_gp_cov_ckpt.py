...
# - Instead, we implement the Exact GP predictive mean and (diagonal) covariance
#   with a Matern-5/2 kernel directly in PyTorch using the learned hyperparameters
#   from the trained GP.
class GPOnnxWrapper(nn.Module):
    def __init__(self, gp, likelihood, train_x, train_y):
        ...
        # Build K_xx + sigma_n^2 I and its inverse once (scaled residual space)
        n = K_xx.shape[0]
        K_xx_noisy = K_xx + (self.noise + 1e-6) * torch.eye(
            n, device=train_x.device, dtype=train_x.dtype
        )
        K_inv = torch.linalg.inv(K_xx_noisy)
        self.register_buffer("K_inv", K_inv)

        # Precompute alpha = (K_xx + sigma_n^2 I)^{-1} y  (in scaled residual space)
        alpha = K_inv @ train_y.unsqueeze(-1)  # (n_train, 1)
        self.register_buffer("alpha", alpha.squeeze(-1))
    ...
    def forward(self, x):
        """
        Predictive mean and (approximate) observation variance at test inputs x.

        Returns:
            mean:     (batch, 1) predictive mean (scaled residual space)
            variance: (batch, 1) predictive variance (scaled residual space)
        """
        # x: (batch, D)
        K_xs = self._matern52_kernel(self.train_x, x)  # (n_train, batch)

        # Mean: K_xs^T (K_xx + sigma_n^2 I)^{-1} y
        alpha = self.alpha.unsqueeze(-1)  # (n_train, 1)
        mean = K_xs.transpose(0, 1) @ alpha  # (batch, 1)

        # Diagonal predictive covariance of f_*:
        #   cov(f_*) = k_ss - K_xs^T K_inv K_xs
        # For Matern-5/2, k(x, x) = outputscale.
        batch_size = x.shape[0]
        k_ss_diag = self.outputscale.expand(batch_size)  # (batch,)

        # K_inv K_xs: (n_train, n_train) @ (n_train, batch) -> (n_train, batch)
        K_inv_K_xs = self.K_inv @ K_xs

        # Diag term: sum_i K_xs(i, j) * (K_inv K_xs)(i, j) for each j
        diag_term = (K_xs * K_inv_K_xs).sum(dim=0)  # (batch,)

        var_f = k_ss_diag - diag_term

        # Add observation noise to get predictive variance of y_*
        var_y = var_f + self.noise

        # Numerical safety: clamp to non-negative
        var_y = torch.clamp(var_y, min=0.0)

        return mean, var_y.unsqueeze(-1)  # both (batch, 1)
...
with torch.no_grad():
    ...
    wrapper_mean_scaled, wrapper_var_scaled = onnx_model(input_tensor)
    wrapper_scaled = wrapper_mean_scaled.squeeze(-1)  # (N,)
...
try:
    ...
    mean_name = ort_session.get_outputs()[0].name
    var_name = ort_session.get_outputs()[1].name
    print(f"  Input name: {input_name}")
    print(f"  Mean output name: {mean_name}")
    print(f"  Var  output name: {var_name}")

    print(f"\nRunning ONNX inference on same test data...")
    onnx_input = realistic_inputs_scaled.astype(np.float32)

    # Single pass for correctness comparison (scaled space)
    onnx_mean_scaled, onnx_var_scaled = ort_session.run(
        [mean_name, var_name], {input_name: onnx_input}
    )
    onnx_mean_scaled = onnx_mean_scaled.reshape(-1)
    onnx_var_scaled = onnx_var_scaled.reshape(-1)

    # PyTorch (GPyTorch) reference in scaled space
    pytorch_mean_scaled = output_scaled.detach().cpu().numpy().reshape(-1)
    pytorch_var_scaled = (
        output_dist.variance.detach().cpu().numpy().reshape(-1)
    )  # predictive var of y_*

    # Comparison: mean and std in scaled space
    print(f"\n{'='*80}")
    print("PyTorch vs ONNX Comparison (scaled space):")
    print(
        f"{'Example':<20} "
        f"{'PyTorch μ':<15} {'ONNX μ':<15} {'|Δμ|':<15} "
        f"{'PyTorch σ':<15} {'ONNX σ':<15} {'|Δσ|':<15}"
    )
    print("-" * 80)
    ...
    max_std_diff = 0.0
    for i, key in enumerate(sample_keys):
        mu_pt = pytorch_mean_scaled[i]
        mu_ox = onnx_mean_scaled[i]
        sigma_pt = float(np.sqrt(max(pytorch_var_scaled[i], 0.0)))
        sigma_ox = float(np.sqrt(max(onnx_var_scaled[i], 0.0)))

        d_mu = abs(mu_pt - mu_ox)
        d_sigma = abs(sigma_pt - sigma_ox)
        max_mean_diff = max(max_mean_diff, d_mu)
        max_std_diff = max(max_std_diff, d_sigma)

        print(
            f"{key:<20} "
            f"{mu_pt:>14.4f} {mu_ox:>14.4f} {d_mu:>14.6f} "
            f"{sigma_pt:>14.4f} {sigma_ox:>14.4f} {d_sigma:>14.6f}"
        )

    print(f"\nMax |Δμ| (scaled):  {max_mean_diff:.8e}")
    print(f"Max |Δσ| (scaled):  {max_std_diff:.8e}")
    if max_mean_diff < 1e-4 and max_std_diff < 1e-4:
        print("✓ ONNX mean and std match PyTorch (scaled space)")
    else:
        print("⚠ Warning: Differences found between PyTorch and ONNX (mean/std)")
    ...
    # PyTorch benchmark (GPOnnxWrapper, mean+var)
    ...
    # ONNX Runtime benchmark (mean+var)
    start = time.time()
    for _ in range(n_iterations):
        _ = ort_session.run([mean_name, var_name], {input_name: onnx_input})
...
try:
    ...
    mean_name = session.get_outputs()[0].name
    var_name = session.get_outputs()[1].name
    ...
    if scaler_x is None or scaler_y is None:
        ...
        print("-" * 80)
        start_time = time.time()
        onnx_mean_scaled, onnx_var_scaled = session.run(
            [mean_name, var_name], {input_name: mock_features_scaled.astype(np.float32)}
        )
        onnx_output_scaled = onnx_mean_scaled  # keep name consistent with above logs
